...
class RotatingHotstuffBFTNode (RotatingLeaderHotstuff, Runnable):
    ...
    def __init__(self, sid, id, S, T, Bfast, Bacs, N, f, bft_from_server: Callable, bft_to_client: Callable,
                 ready: mpValue, stop: mpValue, tx_storage: MempoolClient, K=3, mode='debug', mute=False, bft_running: mpValue=mpValue(c_bool, True),
    ...
    def prepare_bootstrap(self):
        if self.mode == 'test' or 'debug': #K * max(Bfast * S, Bacs)
            self.transaction_buffer.bootstrap(self.id,
